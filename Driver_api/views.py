    ...
    if request.method == 'POST':
        ...
        with connections[school_name].cursor() as cursor:
            ...
            result = {
                ...
                "use_round_order": login_details1[0]['use_round_order'] if login_details1[0][
                    'use_round_order'] else False,
                                ...
                                if rounds_details:
                                    ...
                                    if round_state_student:
                                        ...
                                        for student_state in round_state_student:
                                            type_round = "drop_off" if 'drop' == request.data.get(
                                                'round_type') else "pick_up"
                                            cursor.execute(
                                            ...
                                            if 'drop' == request.data.get('round_type'):
                                                ...
                                                cursor.execute(
                                                    ...
                                                    ['Pick Up By Parent', driver_id, start, datetime.datetime.now(),
                                                     'buses', student_student2[0][0]])
                                                ...
                                                if pickup_request or school_message:
                                                    moved_students.append(student_student2[0][
                                                                              0] + "</font></b><br> , has been picked up by parents")
                                                ...
                                                if school_day_student_round_transfer_rel:
                                                    if not student_state[3]:
                                                        ...
                                                        moved_students.append(
                                                            "The student <br><b><font color='#CE3337'>" + str(
                                                                student_student21[0][
                                                                    0]) + "</font></b><br> has been moved from this bus for this round only")
                                                    else:
                                                        cursor.execute(
                                                            "select display_name_search from student_student WHERE id = %s ",
                                                            [student_state[0]])
                                            ...
                                            if round_history[0][1].strftime('%Y-%m-%d') == str(now):
                                                ...
                                                if student_history:
                                                    ...
                                                    if student_history[0][0] == 'in':
                                                        ...
                                                        cursor.execute(
                                                            "UPDATE public.transport_participant SET transport_state = %s,write_date=%s WHERE student_id =%s AND round_schedule_id= %s",
                                                            ['absent_all', datetime.datetime.now(),
                                                             student_student12[0]['id'],
                                                             rounds_details[0][0]])
                                                else:
                                            ...
                                            "parents_info": [
                                                {
                                                    "father": {
                                                        ...
                                                        mother_inf[0]['mobile'] if mother_inf else "",
                                                    ...
                                                    "mother": {
                                                        ...
                                                        father_inf[0]['mobile'] if father_inf else "",
                                                        "id": mother_inf[0]['id'] if mother_inf else -1
                                                    }
                                                }
                                            ],
                                            "avatar": 'https://trackware-schools.s3.eu-central-1.amazonaws.com/' +
                                                      student_student12[0]['image_url'] if student_student12[0][
...
def save_message_wizard(school_name, round_id, date, from_type, message_title, message_title_ar, message_body,
                        message_body_ar, driver_name, student_id=0):
    with connections[school_name].cursor() as cursor:
        notifications_title = ''
        ...
        cursor.execute(
            ...
            [date, 'App\Model\drive', message_body, message_body_ar, notifications_title, notifications_title, date,
             student_name[0][0], branch_id[0][0], student_id])
                            ...
                            with connections[school_name].cursor() as cursor:
                                ...
                                if status == 'start':
                                    ...
                                    for k in rounds_count_student:
                                        ...
                                        message_body_ar = "بدأت الجولة الصباحية, الرجاء ان يكون " + student_name[0][
                                            0] + " مستعداً"
                                        ...
                                        if round_info1[0][1] == 'pick_up':
                                            ...
                                            if round_history:
                                                ...
                                                if round_history[0][1].strftime('%Y-%m-%d') == str(now):
                                                    ...
                                                    if student_history1:
                                                        if student_history1[0][0] == 'absent' or student_history1[0][
                                                            0] == 'absent-all' or student_history1[0][0] == 'no-show' or \
                                                                student_history1[0][0] == 'in':
                                                            continue
                                                else:
                                                    cursor.execute(
                                                        "select  activity_type,lat,long from student_history WHERE round_id = %s and student_id=%s and datetime >= %s and datetime <= %s   ORDER BY ID DESC LIMIT 1 ",
                                            ...
                                            insert_sql_message_student += str(
                                                (date_string, 'App\Model\drive', message_body, message_body_ar,
                                                 notifications_title, notifications_title, date_string,
                                                 student_name[0][2], branch_id[0][0], k[0])) + ","
                                            ...
                                            for rec in rounds_count_student:
                                                ...
                                                if student_history_yousef:
                                                    if student_history_yousef[0][0] == 'near':
                                                        ...
                                                        cursor.execute(
                                                            "UPDATE public.transport_participant SET transport_state = %s,write_date=%s WHERE student_id = %s AND round_schedule_id= %s",
                                                            [student_history_yousef[0][0], datetime.datetime.now(),
                                                             rec[0],
                                                             rounds_details[0][0]])
                                                else:
...
def check_out(student_id, bus_num, student_name, round_id, driver_name, rec, school_name, parent_id):
    ...
    title_ar = 'اشعار من الحافلة'
    message_ar = 'وصل إلى المنزل.' + student_name
    message = 'The bus ' + str(bus_num) + ' has arrived at your home and ' + \
              student_name + ' has been checked out of the bus. '
    date_string = datetime.datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S")
    r = datetime.datetime.strptime(date_string,
                                   '%Y-%m-%d %H:%M:%S')
...
def near(round_type, bus_num, student_name, school_name, parent_id):
    title = 'Checkout Notification'
    title_ar = 'اشعار من الحافلة'
    message_ar = 'وصل إلى المنزل.' + student_name
    message = 'The bus ' + str(bus_num) + 'has arrived at your home and ' + \
...
def absent(student_id, student_name, round_id, driver_name, rec, school_name, parent_id):
    title = 'Absence notification'
    title_ar = 'اشعار من الحافلة'
    message_ar = "غائب اليوم." + student_name
    message = ' Your child ' + student_name + ' has not checked into the bus and is absent today.'
    date_string = datetime.datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S")
    r = datetime.datetime.strptime(date_string,
                                   '%Y-%m-%d %H:%M:%S')
...
def no_show(student_id, student_name, round_id, driver_name, rec, school_name, parent_id):
    title = ' No Show Notification'
    title_ar = ' إشعار الغياب'
    message_ar = "لم يظهر اليوم." + student_name
    message = student_name + ' did not check into the bus today'
    date_string = datetime.datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S")
    r = datetime.datetime.strptime(date_string,
                                   '%Y-%m-%d %H:%M:%S')
                        ...
                        if name == 'battery_low':
                            with connections[school_name].cursor() as cursor:
                                ...
                                cursor.execute(
                                    ...
                                    [driver_id])
                                driver_id = cursor.fetchall()
                                message_en = "The battery of the tracking device in the bus " + str(
                                    bus_num[0][0]) + " is running out of charge"
                                cursor.execute(
                            ...
                            with connections[school_name].cursor() as cursor:
                                ...
                                message_en = "	The driver " + driver_id[0][
                                    0] + " has stopped in . The allowed time is 1 minute(s)"
                                cursor.execute(
                                    "INSERT INTO sh_message_wizard(create_date,from_type, type, message_en,sender_name)VALUES (%s,%s,%s,%s,%s);",
                            ...
                            with connections[school_name].cursor() as cursor:
                                ...
                                for students in students_ids:
                                    ...
                                    for parent_id in student_student2:
                                        ...
                                        cursor.execute(
                                            "INSERT INTO sh_message_wizard(round_id,create_date,from_type, type, message_en,sender_name,message_ar)VALUES (%s,%s,%s,%s,%s,%s,%s);",
                                            [round_id, r, 'App\Model\sta' + str(parent_id[0]),
                                             'emergency_student' + str(students), emergency_text,
                                             driver_id[0][0], emergency_text])
                                result = {'status': "ok"}
                                return Response(result)
                            ...
                            with connections[school_name].cursor() as cursor:
                                ...
                                message_en = "	The home location of the student " + str(
                                    student_name[0][0]) + " has been changed by the bus  " + str(bus_num[0][0])
                                cursor.execute(
                                    "INSERT INTO sh_message_wizard(create_date,from_type, type, message_en,sender_name)VALUES (%s,%s,%s,%s,%s);",
