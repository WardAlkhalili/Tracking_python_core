...
def hide_message(request):
    if request.method == 'POST':
        if request.headers:
            ...
            result = {'result': 'error4'}
            return Response(result)
def get_info_message_new(deadline, notifications_text, avatar, create_date, notifications_title, student_name, student_id,id=0,stutes=None,show_notif=None):
    show=show_notif
    if show==None:
        show="show"
    notificationsType = ''
...
def get_school_message_new(student_id, school_name, school_message, student_name):
    ...
    with connections[school_name].cursor() as cursor:
        ...
        if message_ids:
            ...
            cursor.execute(
                "select  school_message_id,id,read_message,show_message from school_message_student_student WHERE school_message_id in %s AND student_student_id = %s AND show_message=true",
                ...
                for rec in range(len(school_message1)):
                    ...
                    notifications.append(
                        get_info_message_new(deadline, notifications_text, avatar, create_date, notifications_title,
                                             student_name, student_id,res[1],"Read" if res[2] else 'UnRead',"show" if res[3] else 'not show' ))
