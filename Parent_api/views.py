        ...
        if request.headers:
            if request.headers.get('Authorization'):
                if 'Bearer' in request.headers.get('Authorization'):
                    ...
                    au = request.headers.get('Authorization').replace('Bearer', '').strip()
                    l.append(au.split(","))
                    db_name=ManagerParent.objects.filter(token=au.split(",")[0]).values_list('school_id','parent_id','db_name')
                    db_name_test = []
                    school_name=''
                    if db_name:
                        for e in db_name:
                ...
                if 'Bearer' in request.headers.get('Authorization'):
                    ...
                    if db_name:
                        ...
                        with connections[school_name].cursor() as cursor:
                            ...
                            attached_files = request.data.get("file")
                            body = json.dumps({"jsonrpc": "2.0",
                                               "params": {"student_id": int(user_id_q[0][0]),"attachments": attached_files,"wk_ids": wk_id,}})
                            headers = {
                                'X-Openerp-Session-Id': Session,
                                'Content-Type': 'application/json',
                            }
