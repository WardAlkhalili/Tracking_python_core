...
def get_marks(request, student_id):
    if request.method == 'GET':
        ...
        with connections[db_name].cursor() as cursor:
            ...
            class_id = 0
            # ----------------------
            # Get student's academic grade
            cursor.execute("""
                SELECT academic_grade_id FROM public.student_distribution_line 
                WHERE id = (
            ...
            student_grade = academic_grade_q[0] if academic_grade_q else None

            if not student_grade and student_distribution_line:
                cursor.execute("SELECT id FROM public.academic_grade WHERE id = %s", [student_distribution_line[0]])
                academic_grade = cursor.fetchone()
                student_grade = academic_grade[0] if academic_grade else None

            # Get student's school classes
            cursor.execute("SELECT id FROM public.school_class WHERE academic_grade_id = %s", [student_grade])
            school_classes = [row[0] for row in cursor.fetchall()]

            # Fetch exams related to these classes
            cursor.execute("""
                SELECT mark_exam_id, school_class_id 
                FROM mark_exam_school_class_rel 
                WHERE school_class_id IN %s
            """, [tuple(school_classes)])
            ...
            if mark_eva:
                for semester in academic_semesters:
                    ...
                    for mark in mark_eva:
                        ...
                        if mark_exam and semester == mark_exam[0]:  # Ensure result exists before accessing index
                            cursor.execute("""
                            ...
                            for exam in exam_names:
                                ...
                                for subject_id, max_mark in subject_mark_line:
                                    cursor.execute("SELECT name FROM public.school_subject WHERE id = %s", [subject_id])
                                    ...
                                    if mark_mark_x:
                                        ...
                                        student_mark1 = cursor.fetchone()

                                        # Assign class_id if not already set
                                        if student_mark1 and class_id == 0:
                                            class_id = mark_mark_x[1]

                                    subject_det.append({
                                        "subject_name": subject_name[0] if subject_name else '',
                                        "student_mark": str(student_mark1[0]) if student_mark1 else "0.0",
